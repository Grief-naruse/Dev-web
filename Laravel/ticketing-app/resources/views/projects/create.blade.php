                    ...
                    <div id="teamListContainer" style="overflow-y: auto; flex: 1; padding: 10px 20px;">
                        @foreach($users as $u)
                            <label class="team-member-item" style="display: flex; align-items: center; justify-content: space-between; padding: 10px; border-bottom: 1px solid #f1f5f9; cursor: pointer; transition: 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                                ...
                                <input type="checkbox" name="users[]" value="{{ $u->id }}" data-name="{{ $u->name }}" data-avatar="{{ $u->avatar ? asset('storage/avatars/' . $u->avatar) : '' }}" data-initial="{{ substr($u->name, 0, 1) }}" onchange="updateSelectedTeamUI()" style="transform: scale(1.2); accent-color: #3498db;" @if(in_array($u->id, old('users', []))) checked @endif>
                            </label>
                        @endforeach
...
<script>
    ...
    // Mettre à jour l'affichage sur la page principale quand on coche/décoche
    function updateSelectedTeamUI() {
        displayArea.innerHTML = ''; // On vide l'affichage
        ...
        checkboxes.forEach(cb => {
            if (cb.checked) {
                ...
                badge.innerHTML = `
                    ${imgHtml}
                    <span style="font-size: 0.85rem; font-weight: 600; color: #2c3e50;">${name}</span>
                `;
    ...
    // Initialisation au chargement de la page (pour le old() de Laravel)
