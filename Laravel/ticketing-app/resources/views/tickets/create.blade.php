...
@section('content')
    <div class="container-fluid" style="padding: 20px; max-width: 900px; margin: auto;">
        ...
        <div style="margin-bottom: 20px;">
            <a href="{{ route('tickets.index') }}" style="color: #3498db; text-decoration: none; font-weight: bold;">← Retour à la liste</a>
        </div>
        ...
        <div class="card" style="background: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            <form action="{{ route('tickets.store') }}" method="POST">
                ...
                <div style="margin-bottom: 20px;">
                    <label for="project_id" style="display: block; font-weight: bold; margin-bottom: 8px; color: #2c3e50;">Projet rattaché *</label>
                    <select name="project_id" id="project_id" style="width: 100%; padding: 10px; border: 1px solid #bdc3c7; border-radius: 4px; background-color: #f9f9f9;" required>
                ...
                <div style="margin-bottom: 20px;">
                    <label for="title" style="display: block; font-weight: bold; margin-bottom: 8px; color: #2c3e50;">Sujet du ticket (Résumé) *</label>
                    <input type="text" name="title" id="title" style="width: 100%; padding: 10px; border: 1px solid #bdc3c7; border-radius: 4px;" value="{{ old('title') }}" placeholder="Ex: Bug sur l'export PDF" required>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea name="description" id="description" style="width: 100%; padding: 10px; border: 1px solid #bdc3c7; border-radius: 4px;" rows="5" placeholder="Détaillez votre demande ici...">{{ old('description') }}</textarea>
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <select name="type" id="type" style="width: 100%; padding: 10px; border: 1px solid #bdc3c7; border-radius: 4px;">
                            <option value="included" {{ old('type') == 'included' ? 'selected' : '' }}>📦 Inclus dans le forfait</option>
                            <option value="billable" {{ old('type') == 'billable' ? 'selected' : '' }}>💸 Hors-forfait (Facturable)</option>
                        </select>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div id="selected-team-display" style="display: flex; flex-wrap: wrap; gap: 10px; min-height: 40px; padding: 10px; border: 1px dashed #bdc3c7; border-radius: 6px; background: #fafafa;">
                        <span style="color: #95a5a6; font-size: 0.9rem; font-style: italic; margin: auto 0;">Sélectionnez d'abord un projet pour voir l'équipe...</span>
                    </div>
                ...
                <div style="text-align: right; border-top: 1px solid #ecf0f1; padding-top: 20px;">
                    <button type="submit" style="background-color: #3498db; color: white; padding: 12px 30px; border: none; border-radius: 4px; font-weight: bold; font-size: 1rem; cursor: pointer;">
                        Créer le ticket
                    </button>
                </div>
    ...
    <script>
        ...
        // Aucun membre assigné à la création
